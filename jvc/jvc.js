...
const brandUrl = "https://www.service-center-locator.com/jvc/jvc-service-center.htm"
        ...
        try {
            ...
            rp(brandUrl).then((htmlString) => {
                ...
                $(postDiv).find(" div:nth-child(9),div:nth-child(10)").children("ul").map((i, state) => {
                    jvc[i] = {}
                    jvc[i]['state'] = ($(state).children("strong").text())
                    jvc[i]['states'] = []
                    $(state).children("li").each(async (j, city) => {
                        jvc[i]['states'][j] = {}
                        jvc[i]['states'][j]['name'] = $(city).text()
                        const link = $(city).children("a").attr("href").replace("../", baseUrl)
                        jvc[i]['states'][j]['link'] = link

                        jvc[i]['states'][j]['city'] = await detailsPage(link, $(city).text())
                    })

                })
                setTimeout(() => {
                    const brand = JSON.stringify(jvc)
                    fs.writeFileSync("./jvc/jvc.json", brand)
                }, 8000)
            })
